...
Example code:
    ...
    // You may also pass your own filter function to filterfnoise2d.
    //  Pass a pointer to any function that takes a double and returns a
    //  double. pnoise values are passed through this function before
    //  being summed. The filter function defaults to an absolute-value
    //  function.
    p.filterfnoise2d(&data2d[0][0], N, N, FREQ, FREQ,
                     myFilter);
    p.normalize2d(&data2d[0][0], N, N);
...
#include "grand.h"  // For class GRand - pseudorandom number generation
#include <vector>
using std::vector;
...
public:
    ...
    typedef double (*filter_type)(double);  // Type for filter function
...
public:
                                          ...
                                          // phase shifts
    {
        ...
        addpnoise3d(
            ...
            freqx, freqy, freqz,
            scale,
            shiftx+double(dimx)/(freqx*2),
            shifty+double(dimy)/(freqy*2),
            shiftz+double(dimz)/(freqz*2));
                                          // For consistency w/ fnoise
    }
    ...
    void filterpnoise3d(
        ...
        int dimx, int dimy, int dimz,     // array dims
        int freqx, int freqy, int freqz,  // "frequencies"
        filter_type filter=pnoise_abs,    // filter func for pnoise vals
        double scale=1.,                  // "amplitude"
        double shiftx=0., double shifty=0., double shiftz=0.)
                                          // phase shifts
    {
...
private:
    ...
    void filter3d(
        double * data,                // the 3-D array
        int dimx, int dimy, int dimz, // array dims
        filter_type filter)           // filter function
    {
        const int size = dimx * dimy * dimz;
        for (int i=0; i<size; ++i)
