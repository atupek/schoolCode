...
void setupCars()
{
	...
	{
		carArray[i] = new Car;
		carArray[i].distance = 5.0 * (i%5);
		if(i%3==0)
		{
			carArray[i].speed = 0.20;
			carArray[i].carColorG=255; //green for fast
			carArray[i].carColorR = carArray[i].carColorB = 0;
		}
		else if(i%3==1)
		{
			carArray[i].speed = 0.10;
			carArray[i].carColorB=255; //blue for medium
			carArray[i].carColorG = carArray[i].carColorR = 0;
		}
		else
		{
			carArray[i].speed = 0.05;
			carArray[i].carColorR=255; //red for slow
			carArray[i].carColorG = carArray[i].carColorB = 0;
		}
		carArray[i].desiredSpeed = carArray[i].speed;
		//give each car a random starting segment
		int randSegment = int(random(1000))%34;
		carArray[i].currSegment = segmentArray[randSegment].name;
	}
}
...
void updateCar(Car c, int dt)
{
	...
	int segNum;
  	...
  	//determine which segment to move to next
